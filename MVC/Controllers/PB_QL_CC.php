<?php
class PB_QL_CC extends controller{
    private $dssv;
    function __construct()
    {
        $this->dssv=$this->model('PB_QL_CC_m');
    }
    function Get_data(){
        $this->view('Masterlayout_PB',[
            'page'=>'PB_QL_CC_View',
            'dulieu'=>$this->dssv->pbcc_find('','')
        ]);
    }
    ...
    function fillter() {
        ...
        if ($ten_tk == '' && $filter == '') {
            echo '<script>window.location.href = "http://localhost/congnghephanmem/PB_QL_CC";
            </script>';
        } else {
            // Xử lý lọc dữ liệu
            if ($filter == 'ALL') {
                $dl = $this->dssv->pbcc_find($ten_tk, '');
            } 
            
            else {
                $dl = $this->dssv->pbcc_find($ten_tk, $filter);
            }
